...
def PrintIRQInfo(infile, field, offset, length, version, mode, outfile):
        addr = 0
        row = 0
        i = 0
        MyOffset = eval(offset)
        infile.seek(MyOffset)
        print("got entry 0x400002F PrintIRQInfo, length is 0x%x" %(int(length,16)))
...
def entry_0x400002F(infile, field, offset, length, version, mode, outfile):
    if not field == '0x400002F':
        print('hidis field is ', field)
        print('current field is', '0x400002F')
        return error['ERR_CHECK_FIELD']
    elif not version == '0x0000':
        print('hidis version is ', version)
        print('current version is ', '0x0000')
        return error['ERR_CHECK_VERSION']
    elif not length == '0x100':
        print('hids len is ', length)
        print('current len is ', '0x100')
        return error['ERR_CHECK_LEN']
    print("got entry 0x400002F")

    PrintIRQInfo(infile, field, offset, length, version, mode, outfile)
    return 0
    
if __name__ == '__main__':
    entry_0x400002F(infile, field, offset, length, version, mode, outfile)
