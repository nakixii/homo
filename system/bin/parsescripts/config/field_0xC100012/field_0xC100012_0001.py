...
#
#
#   description     :   rc config for exparse python sripts frame
#
#   modify  record  :   2019-04-25 add user 13 in g_Pcie_UserEnumTable
#                       2019-07-02 add ETH DV2 DMA User and Pre-linkup EP Cb, adjust dma trans_type location.
#######################################################################################################################################
import struct
import os
import sys
...
def parse_pcie_bin_lock(fifo_type, infile, offset, outfile):
    ...
    struct.unpack('I', infile.read(4))#trace-index
    total_offset +=  (4 + 4)
    outfile.writelines('first sign byte 0x52525252, total_offset: 0x%x\n\n' %(total_offset))
    ...
    for i in range(0, 14):#user_id
        ...
        if lock_time > 0 and i > 2:#EP PM User lock time not valid
            outfile.writelines('user: %s, total_lock_time: 0x%-18x\n' %(g_Pcie_UserEnumTable[i], lock_time))
        elif i == 1:
            outfile.writelines('user: %s, total_lock_time refers to PCIE_EP_DBG Analysis\n' %(g_Pcie_UserEnumTable[i]))
...
def parse_pcie_bin_cb(fifo_type, infile, offset, outfile):
    ...
    for i in range(0, 14):#user_id
        for j in range(0, 9):#cb_stage
            ...
            if time_stamp > 0:
                outfile.writelines('%-20s %-20s %-20d 0x%-18x\n' %(g_Pcie_UserEnumTable[i], g_Pcie_ApCbTable[j], cb_result, time_stamp))
...
def parse_pcie_bin_cfg(fifo_type, infile, offset, outfile):
    infile.seek(offset)
    global total_offset
    outfile.writelines("**************************************pcie pm cfg recover debug**************************************\n")
    outfile.writelines('%-20s %-20s %-20s %-20s\n' %('cfg_type', 'func_num','reg_offset', 'reg_val'))
    for i in range(0, 2):#cfg type
        for j in range(0, 8):#func_num
            for k in range(0, 16):#reg_num
                (reg_val, )            = struct.unpack('I', infile.read(4))
                if reg_val > 0:
                    outfile.writelines('%-20s 0x%-18x 0x%-18x 0x%-18x\n' %(g_Pcie_ApCfgTable[i], j, 4*k, reg_val))

    total_offset +=  2*8*16*4
    outfile.writelines('total_offset: 0x%x\n\n' %(total_offset))
...
def parse_pcie_bin_dma_addr(fifo_type, infile, offset, outfile):
    ...
    for i in range(0, 32):#trace list
        ...
        (trace_array[i][3], )   = struct.unpack('I', infile.read(4)) #trans_type
...
def entry_0xC100012(infile, field, offset, len, version, mode, outfile):
        new_offset = eval(offset)
        global total_offset
        ########check parameter start#############
        if not field == '0xC100012':
            print(('hidis field is', field))
            print(('current field is', '0xC100012'))
        ...
        parse_pcie_bin_cfg("cfg_dbg",infile, new_offset + total_offset, outfile)
