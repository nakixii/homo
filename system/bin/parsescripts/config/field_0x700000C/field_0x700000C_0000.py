...
# * Filename
# * Description   analysis userdata dump
# * Version       1.0
# * Data          2019.6.28
#***********************************************************************
import struct
...
def PrintUserData(infile, field, offset, slen, version, mode, outfile):
        ...
        print("got entry 0x700000C PrintUserData")
        outfile.writelines("offset      data\n")
        for i in range(0, int(int(slen, 16)/16) + 1):
...
def entry_0x700000C(infile, field, offset, slen, version, mode, outfile, field_list = None):
        try:
            if not field == '0x700000C':
                print('hidis field is ', field)
                print('current field is', '0x700000C')
