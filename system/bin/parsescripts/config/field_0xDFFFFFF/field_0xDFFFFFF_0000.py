...
def entry_0xDFFFFFF(infile, field, offset, slen, version, mode, outfile):
        try:
            if not field == '0xDFFFFFF':
                print('hidis field is ', field)
                print('current field is', '0xDFFFFFF')
                return error['ERR_CHECK_FIELD']
            elif not version == '0x0000':
                print('hidis version is ', version)
                print('current version is ', '0x0000')
                return error['ERR_CHECK_VERSION']
            elif not slen == '0x135C':
                print('hids len is ', slen)
                print('current len is ', '0x135C')
            print("got entry 0xDFFFFFF")
            print(offset)
