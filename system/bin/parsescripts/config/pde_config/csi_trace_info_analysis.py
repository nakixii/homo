...
def trace_info_analysis(module_bin, timestamp,data_bin, info_name_list, info_data_list,fout_trace_real_info):  
	...
	global line_count
	input_flag = 1#check_name_data(info_name_list, info_data_list, fout_trace_real_info)
...
def analysis_data(module,timestamp): 
	...
	data_bin = 0
	if module_id == 7:
		list_data_trace.append("{} {} {}".format("**************  ISR_MODULE_ANALYSIS (LINE ",str(line_count),") ****************\n"))
		trace_info_analysis(module_bin,timestamp, data_bin, isr.isr_info_name_list, isr.isr_info_data_list,fout_trace_real_info)
	elif module_id == 1:
		list_data_trace.append("{} {} {}".format("**************  CSI_MODULE_ANALYSIS (LINE ",str(line_count),") ****************\n"))
...
def read_list_data(cc_num,list_data):
	...
	for i in range(len(list_data)):
		...
		if param_cnt == 1:
			...
			analysis_data(module, timestamp)
			param_cnt = 0
