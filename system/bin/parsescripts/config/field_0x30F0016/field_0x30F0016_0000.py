...
class BaseInfo:
        ...
        def PrintExcInfo(self,infile,outfile):
                for i in range(0,self.length):
                        ...
                        if tag != b'\x00' and tag < b'\x7F':
                            outfile.writelines(tag)
                return
...
def entry_0x30F0016(infile, field, offset, length, version, mode, outfile):
        try:
            if not field == '0x30F0016':
                print('hidis field is ', field)
                print('current field is', '0x30F0016')
