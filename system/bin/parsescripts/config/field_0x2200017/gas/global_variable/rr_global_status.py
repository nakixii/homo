...
def get_rr_main_state(rr_cur_main_state):
    for index in GAS_RR_MAIN_STATE_ENUM_TABLE.keys():
        if index == rr_cur_main_state:
            return GAS_RR_MAIN_STATE_ENUM_TABLE[index]

    return "none"


def get_rr_sub_state(rr_sub_state):
    for index in GAS_RR_SUB_STATE_ENUM_TABLE.keys():
        if index == rr_sub_state:
            return GAS_RR_SUB_STATE_ENUM_TABLE[index]

    return "none"


def anls_modem_x_rr_glob_sta(instream, file_offset, outstream):
    instream.seek(file_offset)

    (rr_cur_main_state,) = struct.unpack('>B', instream.read(1))
    str_rr_cur_main_state = get_rr_main_state(rr_cur_main_state)
    str_rr_cur_main_state = '%s(0x%x)' % (
        str_rr_cur_main_state, rr_cur_main_state)
    outstream.writelines(
        ["%-15s%-7s\n" % ("rr_cur_main_state :", str_rr_cur_main_state)])

    (rr_cur_sub_state,) = struct.unpack('>B', instream.read(1))
    str_rr_cur_sub_state = get_rr_sub_state(rr_cur_sub_state)
    str_rr_cur_sub_state = '%s(0x%x)' % (
        str_rr_cur_sub_state, rr_cur_sub_state)
    outstream.writelines(
        ["%-15s%-7s\n" % ("rr_cur_sub_state :", str_rr_cur_sub_state)])

    (gcomc_space,) = struct.unpack('>B', instream.read(1))
    (gcomc_space,) = struct.unpack('>B', instream.read(1))

    (rr_cached_event_type,) = struct.unpack('I', instream.read(4))
    outstream.writelines(
        ["%-15s0x%-7x\n" % ("rr_cached_event_type :", rr_cached_event_type)])
