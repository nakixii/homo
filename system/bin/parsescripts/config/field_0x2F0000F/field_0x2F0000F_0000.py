...
def PrintSecDumpInfo(infile, field, offset, slen, version, mode, outfile):
        ...
        print("got entry 0x2F0000F PrintSecDumpInfo, slen is 0x%x" %(int(slen,16)))
...
def entry_0x2F0000F(infile, field, offset, slen, version, mode, outfile):
        try:
            if not field == '0x2F0000F':
                print('hidis field is ', field)
                print('current field is', '0x2F0000F')
                return error['ERR_CHECK_FIELD']
            elif not version == '0x0000':
                print('hidis version is ', version)
                print('current version is ', '0x0000')
                return error['ERR_CHECK_VERSION']
            print("got entry 0x2F0000F")
