...
#                       2019-07-02 add ETH DRV2 DMA User and Pre-linkup Cb, check dma direction valid.
#######################################################################################################################################
import struct
import os
import sys
...
def parse_pcie_bin_dma_addr(fifo_type, infile, offset, outfile):
    ...
    for i in range(0, 32):#trace list
        ...
        (trace_array[i][2], )   = struct.unpack('I', infile.read(4)) #64 bit reserve
        (trace_array[i][3], )   = struct.unpack('I', infile.read(4)) #transfer_type
    ...
    for i in range(0, 32):#trace list
        if trace_array[i][4] > 0:
            if trace_array[i][6] < 2 and trace_array[i][3] < 2:
                outfile.writelines('0x%-18x %-20s %-20s %-20s 0x%-18x 0x%-18x 0x%-18x 0x%-18x 0x%-18x 0x%-18x 0x%-18x\n' \
                %(trace_array[i][4], g_Pcie_DmaChanTable[trace_array[i][5]], g_Pcie_DmaTransTable[trace_array[i][3]], g_Pcie_DmaDirectionTable[trace_array[i][6]], \
                trace_array[i][7], trace_array[i][8], trace_array[i][9], trace_array[i][10], trace_array[i][11], trace_array[i][0], trace_array[i][1]))
            else:
                outfile.writelines('0x%-18x %-20s 0x%-18x 0x%-18x 0x%-18x 0x%-18x 0x%-18x 0x%-18x 0x%-18x 0x%-18x 0x%-18x\n' \
                %(trace_array[i][4], g_Pcie_DmaChanTable[trace_array[i][5]], trace_array[i][3], trace_array[i][6], \
                trace_array[i][7], trace_array[i][8], trace_array[i][9], trace_array[i][10], trace_array[i][11], trace_array[i][0], trace_array[i][1]))

    outfile.writelines('\n\n')
    struct.unpack('I', infile.read(4))#for 64 align
    struct.unpack('I', infile.read(4))#trace-index
    struct.unpack('I', infile.read(4))#for 64 align
    total_offset +=  (32*4*12 + 4 + 4 + 4)
    outfile.writelines('total_offset: 0x%x\n\n' %(total_offset))

def parse_pcie_bin_speed(fifo_type, infile, offset, outfile):
    infile.seek(offset)
    global total_offset
    outfile.writelines("**************************************pcie speed debug**************************************\n")
    outfile.writelines('%-20s %-20s %-20s %-20s\n' %('user_id', 'speed_action','cnt', 'last_time'))
    for i in range(0, 14): #user_id
        for j in range(0, 2): #speed action
            (cnt, )            = struct.unpack('I', infile.read(4))
            (timestamp,   )    = struct.unpack('I', infile.read(4))
            if cnt > 0:
                outfile.writelines('%-20s %-20s %-20d 0x%-18x\n' %(g_Pcie_UserEnumTable[i], g_Pcie_SpeedActTable[j], cnt, timestamp))
    total_offset +=  14*2*(4+4)
    outfile.writelines('total_offset: 0x%x\n\n' %(total_offset))
    outfile.writelines("**************************************pcie speed trace**************************************\n")
    outfile.writelines('%-20s %-20s %-20s %s\n' %('timestamp', 'user_id', 'speed_action', 'volumn(INPUT)/Gen-x(OUTPUT)'))

    trace_array = [[0] * 4 for _ in range(32)]
    for i in range(0, 32):#trace list
        (trace_array[i][0], )    = struct.unpack('I', infile.read(4)) #timestamp
        (trace_array[i][1], )    = struct.unpack('I', infile.read(4)) #user_id
        (trace_array[i][2], )    = struct.unpack('I', infile.read(4)) #speed_action
        (trace_array[i][3], )    = struct.unpack('I', infile.read(4)) #volumn/Gen-x

    sort_array(32,trace_array,0)

    for i in range(0, 32):#trace list
        if trace_array[i][0] > 0:
        	outfile.writelines('0x%-18x %-20s %-20s %-20d\n' %(trace_array[i][0], g_Pcie_UserEnumTable[trace_array[i][1]], g_Pcie_SpeedActTable[trace_array[i][2]], trace_array[i][3]))

    struct.unpack('I', infile.read(4))#trace-index
    total_offset +=  (32*(4+4+4+4) + 4)
    outfile.writelines('total_offset: 0x%x\n\n' %(total_offset))

def parse_pcie_bin_clk(fifo_type, infile, offset, outfile):
    infile.seek(offset)
    global total_offset
    outfile.writelines("**************************************pcie clk debug**************************************\n")
    outfile.writelines('%-20s %-20s %-20s %-20s\n' %('user_id', 'clk_action','cnt', 'last_time'))
    for i in range(0, 14): #user_id
        for j in range(0, 2): #speed action
            (cnt, )            = struct.unpack('I', infile.read(4))
            (timestamp,   )    = struct.unpack('I', infile.read(4))
            if cnt > 0:
                outfile.writelines('%-20s %-20s %-20d 0x%-18x\n' %(g_Pcie_UserEnumTable[i], g_Pcie_ClkActTable[j], cnt, timestamp))
    total_offset +=  14*2*(4+4)
    outfile.writelines('total_offset: 0x%x\n\n' %(total_offset))
    outfile.writelines("**************************************pcie clk trace**************************************\n")
    outfile.writelines('%-20s %-20s %-20s %s\n' %('timestamp', 'user_id', 'clk_action', 'clk_value'))

    trace_array = [[0] * 4 for _ in range(32)]
    for i in range(0, 32):#trace list
        (trace_array[i][0], )    = struct.unpack('I', infile.read(4)) #timestamp
        (trace_array[i][1], )    = struct.unpack('I', infile.read(4)) #user_id
        (trace_array[i][2], )    = struct.unpack('I', infile.read(4)) #clk_action
        (trace_array[i][3], )    = struct.unpack('I', infile.read(4)) #clk value

    sort_array(32,trace_array,0)

    for i in range(0, 32):#trace list
        if trace_array[i][0] > 0:
        	outfile.writelines('0x%-18x %-20s %-20s %-20d\n' %(trace_array[i][0], g_Pcie_UserEnumTable[trace_array[i][1]], g_Pcie_ClkActTable[trace_array[i][2]], trace_array[i][3]))

    struct.unpack('I', infile.read(4))#trace-index
    total_offset +=  (32*(4+4+4+4) + 4)
    outfile.writelines('total_offset: 0x%x\n\n' %(total_offset))

def parse_pcie_bin_slice(fifo_type, infile, offset, outfile):
    infile.seek(offset)
    global total_offset
    outfile.writelines("**************************************pcie slice debug**************************************\n")
    (modem_slice, ) = struct.unpack('I', infile.read(4))
    (ap_slice,   )  = struct.unpack('I', infile.read(4))

    outfile.writelines('modem_slice: 0x%-18x\n' %(modem_slice))
    outfile.writelines('ap_slice: 0x%-18x\n' %(ap_slice))
    total_offset +=  (4+4)
    outfile.writelines('total_offset: 0x%x\n\n' %(total_offset))
...
def entry_0x3100009(infile, field, offset, len, version, mode, outfile):
        ...
        parse_pcie_bin_speed("speed_dbg",infile, new_offset + total_offset, outfile)
        parse_pcie_bin_clk("clk_dbg",infile, new_offset + total_offset, outfile)
        parse_pcie_bin_slice("slice_dbg",infile, new_offset + total_offset, outfile)
        return 0
