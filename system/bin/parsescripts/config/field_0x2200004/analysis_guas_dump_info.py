...
from guas_pid import guas_get_modem0_pid_str
from guas_pid import guas_get_modem1_pid_str
...
def get_was_msg_str_vers0(str_send_pid, msg_id):
    if 'wcom' == str_send_pid.lower() or 'wrr' == str_send_pid.lower():
        return get_was_send_msg_str(msg_id)
    elif 'gas' == str_send_pid.lower():
        return get_gas_was_msg_str(msg_id)
    elif 'wphy' == str_send_pid.lower():
        return get_was_wphy_msg_str(msg_id)
    elif 'timer' == str_send_pid.lower():
        return get_was_timer_msg_str(msg_id)
    elif 'pdcp' == str_send_pid.lower():
        return get_was_pdcp_msg_str(msg_id)
    elif 'rlc' == str_send_pid.lower():
        return get_was_rlc_msg_str(msg_id)
    elif 'mac' == str_send_pid.lower():
        return get_was_mac_msg_str(msg_id)
    elif 'msp_pid' == str_send_pid.lower():
        return get_was_om_msg_str(msg_id)
    elif 'errc' == str_send_pid.lower() or 'ermm' == str_send_pid.lower():
        return get_was_lrrc_msg_str(msg_id)
    elif 'mmc' == str_send_pid.lower():
        return get_gas_nas_msg_str(msg_id)
    elif 'mm' == str_send_pid.lower():
        return get_gas_nas_msg_str(msg_id)
    elif 'gmm' == str_send_pid.lower():
        return get_gas_nas_msg_str(msg_id)
    elif 'bmc' == str_send_pid.lower():
...
def anls_was_per_save_exc_msg_ver0(instream, file_local_offset, outstream):
    ...
    str_send_pid = guas_get_modem0_pid_str(send_pid)
    str_msg_id = get_was_msg_str_vers0(str_send_pid, msg_id)
...
def anls_was_save_nas_msg_vers_02(instream, file_offset, outstream,
    ...
    while looper < MACRO_WAS_MNTN_REC_NAS_MSG_INFO_MAX_CNT:
        looper_index = (
            looper + \
            handled_msg_num) % MACRO_WAS_MNTN_REC_NAS_MSG_INFO_MAX_CNT
        file_local_offset = file_offset + \
            looper_index * MACRO_WAS_SAVE_EXC_MSG_INFO_SIZE
        anls_was_per_save_exc_msg_ver1(
            instream, file_local_offset, outstream, version)
...
def analysis_guas_dump_info(infile, offset, outfile, vers_no):
    instream = infile
    outstream = outfile
    file_offset = eval(offset)
    print("analysis_guas_dump_info")
    ...
    # #### was modem0 #########
    file_offset = file_offset + \
        MACRO_GAS_DEBUG_VERSION_LENGTH + MACRO_GAS_MODEM0_DUMP_INFO_LENGTH
    if MACRO_VERSION_NO_2 <= vers_no:
        anls_was_modem0_dump_vers_02(instream, file_offset, outstream, 0)
    else:
        anls_was_modem0_dump_vers_01(instream, file_offset, outstream)
    ...
    if MACRO_VERSION_NO_4 > vers_no:
        ...
        # #### was modem1 #########
        modem1_was_offset = file_offset + MACRO_GAS_MODEM0_DUMP_INFO_LENGTH
        if MACRO_VERSION_NO_2 <= vers_no:
            anls_was_modem0_dump_vers_02(instream, modem1_was_offset,
                                         outstream, 1)
        else:
            anls_was_modem0_dump_vers_01(
               instream, modem1_was_offset, outstream)
