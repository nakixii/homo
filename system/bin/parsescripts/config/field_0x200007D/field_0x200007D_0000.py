...
def PrintTaskSwitch(infile, field, offset, slen, version, mode, outfile):
        ...
        g_tasklist = []
        print("got entry 0x200007D PrintTaskSwitch")
        ...
        if g_magic==0xabcd6789:
                ...
                print("got entry 0x200007D PrintTaskSwitch dot 1, itemcnt is %d" %(itemcnt))
                ...
                for i in range(0, itemcnt - 1):
                    ...
                    #outfile.writelines("g_flag1 0x%08x g_timestamp1 0x%08x startpoint 0x%08x \n" %(g_flag1, g_timestamp1, startpoint) )
                    if g_timestamp1 < g_timestamp0:
                        #outfile.writelines("found start point  0x%x g_flag1 0x%x g_timestamp1 0x%x\n" %(startpoint,g_flag1,g_timestamp1) )
                        break
                    else:
                        g_tasklist.append(g_task)
                        g_timestamp0 = g_timestamp1
                #output from start point to end
                #outfile.writelines("startpoint 0x%08x itemcnt 0x%08x\n" %(startpoint, itemcnt) )
                ...
                return
        print("got entry 0x200007D PrintTaskSwitch dot 2")
...
def PrintTaskSwitchOld(infile, field, offset, slen, version, mode, outfile):
        ...
        if g_magic==0xabcd6789:
                ...
                return
        print("got entry 0x200007D PrintExcInfo dot 2")
        outfile.writelines('magic %u is wrong' % g_magic)
        return

def entry_0x200007D(infile, field, offset, slen, version, mode, outfile):
        try:
            if not field == '0x200007D':
                print('hidis field is ', field)
                print('current field is', '0x200007D')
