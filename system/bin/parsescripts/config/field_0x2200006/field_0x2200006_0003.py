...
def analysis_gunas_mntn_log_PsDomainInfo_info_version0x02( instream, outstream):
        ...
        outstream.writelines(["LastNrRegTai_PlmnId_0:%X\n" % struct.unpack('B', instream.read(1))])
        outstream.writelines(["LastNrRegTai_PlmnId_1:%X\n" % struct.unpack('B', instream.read(1))])
        outstream.writelines(["LastNrRegTai_PlmnId_2:%X\n" % struct.unpack('B', instream.read(1))])
        (ucReserve,)           = struct.unpack('B', instream.read(1))
        outstream.writelines(["ucReserve:%X\n" % ucReserve])
        
        outstream.writelines(["LastNrRegTai_Tac_Tac:%X\n" % struct.unpack('B', instream.read(1))])
        outstream.writelines(["LastNrRegTai_Tac_TacCont:%X\n" % struct.unpack('B', instream.read(1))])
        (ucReserve,)           = struct.unpack('B', instream.read(1))
        outstream.writelines(["ucReserve:%X\n" % ucReserve])
        (ucReserve,)           = struct.unpack('B', instream.read(1))
        outstream.writelines(["ucReserve:%X\n" % ucReserve])
...
def entry_0x2200006(infile, field, offset, len, version, mode, outfile):
        ...
        elif not version == '0x0003':
            print ("hidis version is %s" % (version))
            print ("current version is 0x0003")
