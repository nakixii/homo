...
def PrintSysctrlInfo(infile, field, offset, length, version, mode, outfile):
        ...
        print("got entry 0x4000069 PrintSysctrlInfo, length is 0x%x" %(int(length,16)))
...
def entry_0x4000069(infile, field, offset, length, version, mode, outfile):
    if not field == '0x4000069':
        print('hidis field is ', field)
        print('current field is', '0x4000069')
    ...
    print("got entry 0x4000069")
    PrintSysctrlInfo(infile, field, offset, length, version, mode, outfile)
    return 0
    
if __name__ == '__main__':
    entry_0x4000069(infile, field, offset, length, version, mode, outfile)
